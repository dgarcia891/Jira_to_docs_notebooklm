export function scrubJiraUI(text, fieldName) {
  if (!text) return "";
  const noise = [
  ...
  let cleaned = text;
  noise.forEach((n) => {
    cleaned = cleaned.replace(new RegExp(n, "gi"), "");
  });
...
export function cleanAssignee(text) {
...
export function findFieldByLabel(doc, label) {
  ...
  if (labelEl) {
    ...
    const value = parent?.textContent?.replace(label, "").trim() || container?.textContent?.replace(label, "").trim();
    return value || "";
  }
  return "";
}
export function extractDescriptionHeuristic(doc) {
...
export function extractAssigneeHeuristic(doc) {
...
export function cleanCommentBody(html, document) {
  const tempDiv = document.createElement("div");
  tempDiv.innerHTML = html;
  const cards = tempDiv.querySelectorAll(
    '[data-node-type="inlineCard"], [data-node-type="blockCard"], .jira-issue-card, .smart-link, [data-testid*="inline-card"], [data-testid*="issue.views.issue-base.content.issue-links"]'
  );
  cards.forEach((card) => {
...
export function scrubCommentText(text) {
  if (!text) return "";
  const uiNoise = [
    "Add a comment…",
    "Add a comment",
